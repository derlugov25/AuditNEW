...
const MOVEMENT_SNIPPETS: Snippet[] = [
  {
    ...
    detail: "Мышцы - главный орган, который помогает организму обращаться с сахаром в крови. После 30 лет без активности мышечная масса теряется по 3-5% за каждые 10 лет, а вместе с ней - обмен веществ и иммунитет.",
    actionByBarrier: {
...
const OPTIMIZATION_BY_DOMAIN: Record<DomainKey, OptimizationVariant[]> = {
  sleep: [
    {
      ...
      action: "2 недели с Oura/Whoop + ежедневный HRV-трекинг. Один «CGM week», чтобы увидеть, как ночная глюкоза влияет на глубокий сон.",
    },
    {
    ...
    },
    {
      ...
      body: "Когда длительность стабильна, следующий рычаг - структура: глубокий SWS-сон в первой половине ночи и REM во второй. Тренируется через предсонную температуру и свет.",
  ...
  nutrition: [
    ...
    },
    {
      ...
      body: "На вашем уровне калорийность обычно в порядке. Следующий рычаг - структура приёмов пищи: окно еды, белок утром, клетчатка с каждым приёмом.",
  ...
  habits: [
    {
      ...
      action: "Ежеквартальный self-audit: алкоголь за 30 дней, никотин (включая случайный), каннабис, снотворные. Любой дрейф >2× от baseline - сигнал к разговору с коучем.",
    },
    {
...
export function yearsLostLineFromDomain(d: DomainScore): string {
  ...
  return tr(
    `Минус ≈${y.toFixed(1)} ${lifeYearsUnitWord(y)} здоровья`,
...
> = {
  ...
  light: {
    withDevices:
      "Точечные рычаги уже видны. Мы найдём, что именно влияет на ваш сон, HRV и энергию, и предложим 2-3 протокола на ближайшие месяцы.",
    noDevices:
      "Точечные рычаги уже видны. Longy ведёт без обязательной покупки гаджетов - начнём с того, что вы можете отметить сами в приложении.",
  },
  moderate: {
    withDevices:
...
export function coverSubtitle(score: ScoreResult): string {
  ...
  switch (band) {
    ...
    case "risk":
      return "Несколько факторов создают дефицит здоровья. Ниже - топ-3 рычага и с чего начать в первую очередь.";
    case "critical":
      return "Несколько факторов создают дефицит здоровья. Ниже - топ-3 рычага и с чего начать в первую очередь.";
  }
}
...
export function goalDomainHeadline(
  ...
  const STRENGTH: Record<DomainKey, string> = {
    ...
    stress: tr(
      "Ментальная устойчивость - ваш рычаг. Пока стресс в норме, цель достигается быстрее.",
  ...
  const BLOCKER: Record<DomainKey, string> = {
    ...
    movement: tr(
      "Активность напрямую определяет, достижима ли цель без плато и спадов энергии.",
      "Movement capacity directly determines whether you can reach this goal without plateaus.",
    ),
    nutrition: tr(
      "Без понятного режима питания остальные усилия дают 20-30% от возможного - это первый рычаг.",
...
export const EIGHT_WEEK_PROMISE: Record<DomainKey, EightWeekBundle[]> = {
  ...
  movement: [
    ...
    [
      ...
      "AI-коуч следит за прогрессией, чтобы не уйти ни в плато, ни в перетрен",
    ],
    [
  ...
  sleep: [
    ...
    [
      ...
      "Температура спальни, свет, последний приём пищи - каждый рычаг проверяется",
...
const LIFE_HACKS: Record<DomainKey, LifeHack[]> = {
  ...
  stress: [
    ...
    },
    {
      ...
      hack: "Холодная жидкость на язык активирует тот же нерв, что и медленное дыхание. Через 30 секунд пульс начнёт снижаться, через 2 минуты - почувствуете, что эмоции отпускают.",
  ...
  movement: [
    ...
    {
      title: "Носите тяжёлые сумки - это тренировка тоже",
      hack: "Не оставляйте тяжелые продукты в машине, чтобы потом «удобно занести». Перенос двух пакетов 100 метров - отличная функциональная нагрузка для рук, корпуса и хвата.",
      why: "Сила хвата - независимый предиктор продолжительности жизни. Каждые 5 кг хвата = -16% риска смертности (Leong, Lancet 2015).",
    },
  ],
  nutrition: [
    {
