...
export function coverCTA(score: ScoreResult): string {
  ...
  if (score.isGainBranch) {
    ...
    return `Ваша база уже сильная. В Longy подключаем данные с устройств, нутригенетику и маркеры воспаления — и добираем ещё ~${gain} ${gain === 1 ? "год" : gain < 5 ? "года" : "лет"} здоровой жизни через precision-настройку поверх образа жизни.`;
  }
  if (y < 3) {
...
export function longyScoreExplanation(): string {
  return "Longy Health Score — оценка вашего образа жизни от 0 до 100. Учитывает 5 факторов, сильнее всего влияющих на скорость старения по исследованию Li et al., 2024 (Harvard Medical School, n > 2 млн). Чем выше — тем медленнее ваш организм изнашивается относительно паспортного возраста.";
}
